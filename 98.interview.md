...
如果一个`main`导入其他的包，包会被顺序导入

2、如果导入的包（`pkg1`）依赖其他的包（包`pkg2`），会首先导入`pkg2`，然后初始化`pkg2`中的常量与变量，如果pkg2中有init函数，会自动执行init

所有包导入完成后才会对`main`的常量和变量进行初始化，然后执行`main`中的`init`函数（如果有的话），最后执行`main`函数
...
1、与byte.Buffer思路类似，既然 string 在构建过程中会不断的被销毁重建，那么就尽量避免这个问题，底层使用一个 buf []byte 来存放字符串的内容。
...
https://liudanking.com/performance/golang-strings-builder-%E5%8E%9F%E7%90%86%E8%A7%A3%E6%9E%90/
...
2个结构体 一个是hmap 一个是bucket。bucket有一个指针，会形成一个链表结构
...
concurrent map的原理 是拆分大map为小map，类似表锁到行锁的优化    写多读少的场景适合
...
https://mp.weixin.qq.com/s/r9yC78exkDbXs7Et2pC5PA
...
Gin为什么不支持MVC ？ 小项目用mvc比较复杂  

​		经常用到的功能支持 支持渲染 静态文件服务  路由
...
第一个原因是，基于反射的代码是比较脆弱的。对于每一个会导致编译器报告类型错误的问题，在反射中都有与之相对应的误用问题

第二个原因是，反射的操作不能做静态类型检查
...
​	https://blog.csdn.net/qq_35554975/article/details/88365913

​	https://blog.caojun.xyz/posts/gomodules/?hmsr=toutiao.io&utm_medium=toutiao.io&utm_source=toutiao.io

​	https://blog.csdn.net/Struggle_PAN/article/details/87158721
...
https://github.com/KeKe-Li/golang-interview-questions
