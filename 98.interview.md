...
性能优化工具  bench pprof     优化思想 ： 要兼顾代码可读性，不要盲目优化
...
非抢占式多任务处理、由协程主动交出控制权 （线程抢占式多任务处理  os随时可以切换，考虑切换保存上下文）
...
### 缓冲channel和无缓冲channel的区别

无缓冲的channel是同步的，必须同时有goroutine发送和接受消息，如果没有发送或者接受会阻塞

有缓存的channel是异步的，发送和接受不会阻塞
